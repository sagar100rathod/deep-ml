...
def test_precision_recall_multiclass_without_reduction():
    ...
    assert pytest.approx(precision(probs, gt), 0.01) == torch.tensor([[0.6667, 0.5000, 0.5000]])
    assert pytest.approx(recall(probs, gt), 0.01) == torch.tensor([[0.5000, 0.6667, 0.5000]])
...
def test_precision_recall_multiclass_macro_reduction():
    ...
    precision = Precision(mode="multiclass", num_classes=3, reduction="macro", class_weights=[1, 0, 0])
    recall = Recall(mode="multiclass", num_classes=3, reduction="macro", class_weights=[1, 0, 0])
...
def test_precision_recall_multilabel():
    gt_class_0 = torch.tensor([ [1, 0, 1],
                                [0, 1, 1],
                                [0, 0, 0]
                              ])

    gt_class_1 = torch.tensor([ [1, 0, 0],
                                [1, 1, 0],
                                [1, 0, 0]
                                ])
    gt_class_2 = torch.tensor([ [1, 1, 0],
                                [0, 1, 1],
                                [0, 0, 0]
                            ])
    gt = torch.stack([gt_class_0, gt_class_1, gt_class_2], dim=0).unsqueeze(dim=0)

    pred_class_0 = torch.tensor([[0, 1, 1],
                                [1, 0, 1],
                                [0, 1, 0]
                                ])

    pred_class_1 = torch.tensor([[1, 0, 0],
                                 [1, 1, 1],
                                 [0, 0, 0]
                                 ])

    pred_class_2 = torch.tensor([[1, 0, 1],
                                 [0, 0, 0],
                                 [1, 1, 1]
                                 ])

    pred = torch.stack([pred_class_0, pred_class_1, pred_class_2], dim=0).unsqueeze(dim=0)


    precision = Precision(mode="multilabel", num_classes=3, threshold=0.6)
    recall = Recall(mode="multilabel", num_classes=3, threshold=0.6)

    assert pytest.approx(precision(pred, gt), 0.001) == torch.tensor([[0.40, 0.75, 0.20]])
